...
public class ParseFeed {
	...
	public static List<PointFeature> parseEarthquake(PApplet p, String fileName) {
		...
		for (int i = 0; i < itemXML.length; i++) {
				...
				// Sets datetime if existing
				String summaryval = getStringVal(itemXML[i], "summary");
				if (titleStr != null) {
				String ans = summaryval;
				String html = ans.substring(9, ans.length() - 3);
				org.jsoup.nodes.Document doc = Jsoup.parse(html);
				Element dts = doc.select("dd").first();
				String ans1 = (dts.text());
				//System.out.println(ans1);
				point.putProperty("datetime", ans1);
				}
